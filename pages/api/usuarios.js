...
export default (req, res) => {
  ...
  switch (req.method) {
    ...
    default:
      res
        .status(200)
        .json(`Request ${req.method} no está autorizado`)
  }
}
...
const handlePutRequest = async (req, res) => {
  try {
    ...
    if (action === "removerlibro") {
      ...
      const removerLectura = usuario.lecturasGuardadas.find(l => l.lectura._id = lecturaId)
...
const handlePostRequest = async (req, res) => {
  ...
  if (action === "registrar") {
    ...
    if (usuario) {
      return res
        .status(422)
        .send(`Usuario con el correo ${correo} ya existe`)
    }
    ...
    if (!usuario) {
      return res
        .status(404).send("El usuario no existe")
    }

    const passwordsIguales = await bcrypt.compare(contrasena, usuario.contrasena)
