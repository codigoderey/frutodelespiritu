...
const lectura = ({ lectura, usuario }) => {
	const { updatedAt, createdAt, publicadoPor, contenido, titulo } = lectura;
	...
	useEffect(() => {
		if (!usuario) {
			...
			const siEsGuardada = usuario.lecturasGuardadas.find(
				(l) => l.lectura._id === lectura._id
			);
			if (siEsGuardada) {
				setBookmark(true);
			}
	...
	return (
		<>
			<Head>
				...
				<meta
					name='keywords'
					content='estudios biblicos, estudia la biblia, entiende la biblia, espiritu santo'
				/>
				<meta
					property='og:title'
					content={`${lectura.titulo} | Fruto del Espíritu`}
				/>
				...
				<meta
					property='og:url'
					content={`${baseURL}/lecturas/lectura?slug=${lectura.slug}`}
				/>
			</Head>
			<LecturaTemplateHeader
				createdAt={createdAt}
				updatedAt={updatedAt}
				escritoPor={publicadoPor.nombre}
				usuarioId={publicadoPor._id}
			/>
			<div className='container-full p2'>
				<Column70>
					<h1 className='lectura-title'>{titulo}</h1>
					<div
						dangerouslySetInnerHTML={{ __html: contenido }}
						className='lectura-single-container p3'></div>
					{usuario && !bookmark ? (
						<div className='guardar-lectura p3 bg-light'>
							<svg
								...
								height='32'
								viewBox='0 0 24 24'>
								<path d='M16 10.975v13.025l-6-5.269-6 5.269v-24h6.816c-1.123 1.168-1.816 2.752-1.816 4.5 0 3.736 3.162 6.768 7 6.475zm4-6.475c0 2.485-2.018 4.5-4.5 4.5-2.484 0-4.5-2.015-4.5-4.5s2.016-4.5 4.5-4.5c2.482 0 4.5 2.015 4.5 4.5zm-2-.5h-2v-2h-1v2h-2v1h2v2h1v-2h2v-1z' />
							</svg>
					...
					{!usuario && (
						<div className='guardar-lectura p3 bg-light'>
							...
							<p className='mb0 ml3'>
								<Link href='/ingresar'>
									<a>Inicia sesión para que guardes esta lectura</a>
								</Link>
							</p>
						</div>
					)}
